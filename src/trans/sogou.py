...
def parse():
    ...
    parser = argparse.ArgumentParser(
        description='the usage of Sougou-Dict command line')
    parser.add_argument('-v', '--version', action='store_true',
                        help="show current version")
    parser.add_argument('word', nargs='*',
                        help='word or phrase you want to translate ')
    return parser
...
def translate(word, *args, **kwargs):
    ...
    payload = "from={}&to={}&pid={}&q={}&sign={}&salt={}".format(
        type_from, type_to, PID, q, sign, salt)
